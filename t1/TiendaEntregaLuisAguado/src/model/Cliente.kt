...
class Cliente(var id: Int, var nombre: String) {
    ...
    fun borrarElementosAuto(id: Int, modo: String = "1") {
        val listaFiltrada = carrito.filter { it.id == id }

        if (listaFiltrada.isEmpty()) {
            println("No se puede borrar, no hay coincidencias.")
        } else if (listaFiltrada.size == 1 || modo.equals("1", true)) {
            carrito.remove(listaFiltrada.first())
            println("Borrado correctamente (uno).")
        } else if (modo.equals("n", true)) {
            carrito.removeAll(listaFiltrada)
            println("Borrados todos los productos con ID $id.")
        } else {
            // Por si pasan otros valores, por defecto borrar uno
            carrito.remove(listaFiltrada.first())
            println("Borrado correctamente (uno).")
        }
    }
